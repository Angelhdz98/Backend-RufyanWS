...
public class ProductDomainFactory {
    ...
    public ProductDomain createProduct(ProductSpecs productSpecs, ProductDomainDetails productDetails){
        return switch (productDetails.getProductType()){
            case PAINTING ->   {
               ...
               if(!(productSpecs.productStock() instanceof PaintingStockDTO paintingStock)){
                ...
                yield new PaintingDomain(0,productSpecs.name(),new PaintingStockManager(paintingStock.availableCopies(),paintingStock.copiesMade(),paintingStock.isOriginalAvailable()), new PaintingPriceManager(paintingPricing.pricePerCopy(),paintingPricing.pricePerOriginal()), productSpecs.productImages(), paintingDetails );
            }
            case CUP -> null;
            case CLOTHING -> null;
            case PRINT -> null;
            default -> throw new IllegalArgumentException("No hay ningun tipo de producto llamado así las opciones son "+ Arrays.toString(ProductType.values()) );
        };
